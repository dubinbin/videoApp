<template>
  <div class="about">
    <div class="aboutTop" v-if="user.name===''|| user.name==null">
    ...
    <group>
      <cell v-if="user.name!==''&& user.name!=null" title="我的收藏" is-link link="/collect"></cell>
      <cell title="历史记录" is-link link="/historyList"></cell>
      <cell v-if="user.name!==''&& user.name!=null" title="设置" is-link  link="/setting"></cell>
      <cell title="意见反馈" is-link link="/feedback"></cell>
    </group>
    
     <span v-if="user.name!==''&& user.name!=null" class="showConfirm" @click="showConfirm">退出登陆</span>
   </div>
  </div>
</template>
